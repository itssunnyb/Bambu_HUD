...
void handleTouch() {
  ...
  int screenX = map(p.x, 3900, 700, 0, 480);
  int screenY = map(p.y, 3900, 350, 0, 320);

  screenX = constrain(screenX, 0, 480);
...
void drawGradientBackground() {
  ...
  for (int y = 0; y < 320; y++) {
    uint16_t c = blend565(r1, g1, b1, r2, g2, b2, y, 319);
...
void drawBottomButtons() {
  tft.fillRoundRect(6, 285, 468, 32, 8, themePanelDark());
  tft.drawRoundRect(6, 285, 468, 32, 8, themeLine());
  ...
  tft.drawString("HUD", 24, 294);

  tft.setTextColor(printerColor, themePanelDark());
  tft.drawString("Print", 132, 294);

  tft.setTextColor(settingsColor, themePanelDark());
  tft.drawString("Set", 260, 294);

  tft.setTextColor(statsColor, themePanelDark());
  tft.drawString("Stats", 365, 294);

  tft.drawFastVLine(123, 289, 24, themeLine());
  tft.drawFastVLine(240, 289, 24, themeLine());
  tft.drawFastVLine(357, 289, 24, themeLine());
}
uint16_t amsColorToTFT(String color) {
  color.replace("#", "");
  color.trim();
